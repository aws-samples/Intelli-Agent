...
def get_llm_processed_prompts(initial_prompt: str):
    ...
    # todo need to check the length of raw_completion
    if len(raw_completion) == 3:
        positive_prompt = raw_completion[0].split(':')[1].strip()
        negative_prompt = raw_completion[1].split(':')[1].strip()
        model_list = raw_completion[2].split(':')[1].strip().split(',')

    if len(raw_completion) == 5:
        positive_prompt = raw_completion[2].split(':')[1].strip()
        negative_prompt = raw_completion[3].split(':')[1].strip()
        model_list = raw_completion[4].split(':')[1].strip().split(',')

    if len(model_list) > 0:
        model_list = model_list[0].replace('[', '').replace(']', '').replace('"', '').split(' ')
