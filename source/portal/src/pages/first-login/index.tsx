import { Button, FormField, Grid, Input, Link, Select, SpaceBetween } from '@cloudscape-design/components';
...
const FIRSTLOGIN: FC = () => {
  ...
  return (
    <div className="pwd-div">
      <div className='container'>
        ...
        <div className='tab' style={{paddingLeft:'10%'}}>
          ...
          <div style={{color:"#000000a6",fontSize:18, fontWeight:800, marginBottom:20}}>{t('auth:first-login.title')}</div>
          <div style={{width:'100%'}}>
            
            <div style={{marginTop:15}}>
                <FormField
                  description={t('auth:first-login.oidcDesc')}
                  label={t('auth:first-login.oidc')}
                >
                  <div className='item'>
                    <Select
                      placeholder={t('auth:first-login.oidcPlaceholder').toString()}
    ...
    <FormField
      description={t('auth:first-login.usernameDesc')}
      label={t('auth:first-login.username')}
    >
      <Input
        value={username}
        placeholder={t('auth:first-login.usernamePlaceHolder').toString()}
        onChange={event =>
      ...
      />
        ...
        <div className='button-group'>
          <Button variant="primary" className='send-email' onClick={()=>{sendEmail()}}>{t('auth:first-login.send')}</Button>
        </div>
        <div style={{color: 'rgb(128, 128, 128)', fontSize: 14,marginTop: 30, width:'90%'}}>
          ...
          <Grid gridDefinition={[{colspan:4},{colspan:8}]}>
            ...
            <div style={{paddingTop:15, textAlign:'right'}}>
              <span style={{color: 'rgb(128, 128, 128)'}}>{t('auth:first-login.needAccount')}</span>
              <Link onFollow={toRegister}>{t('auth:first-login.register')}</Link>
              <span style={{color: 'rgb(128, 128, 128)'}}> {t('auth:first-login.or')} </span>
              <Link onFollow={toLogin}>{t('auth:first-login.login')}</Link>
            </div>
          </Grid>):(
