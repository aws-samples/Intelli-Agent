...
def query_expansion_postprocess(r):
...
def get_query_expansion_chain(
    ...
    chain = RunnableLambda(lambda x: query_expansion_template.invoke({"question": x["query"]})) | llm | RunnableLambda(query_expansion_postprocess)
    return chain