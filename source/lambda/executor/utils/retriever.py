...
class QueryQuestionRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(
        ...
        query_repr = get_similarity_embedding(
            query, self.embedding_model_endpoint, self.model_type
        )
...
class QueryDocumentKNNRetriever(BaseRetriever):
    ...
    def __init__(
        self, workspace, using_whole_doc, context_num, top_k, query_key="query"
    ):
        super().__init__()
        self.index = workspace["open_search_index_name"]
        self.vector_field = "vector_field"
    ...
    def _get_relevant_documents(
        ...
        query_repr = get_relevance_embedding(
            query, self.lang, self.embedding_model_endpoint, self.model_type
        )
