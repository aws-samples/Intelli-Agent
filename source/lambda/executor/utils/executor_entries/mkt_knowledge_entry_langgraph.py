...
from ..constant import CONVERSATION_SUMMARY_TYPE, IntentType
...
from ..langchain_utils import RunnableDictAssign, RunnableParallelAssign, chain_logger
...
def get_intent_recognition_with_index_chain(state):
    ...
    inten_postprocess_chain = intent_recognition_index.as_intent_postprocess_chain(
        method="top_1"
    )

    intent_search_and_postprocess_chain = (
        intent_index_search_chain | inten_postprocess_chain
    )
...
def get_qq_match_chain(state):
    ...
    qq_chain = (
        ...
        | RunnableLambda(
            partial(retriever_results_filter, threshold=qq_match_threshold)
        )
    )
    return qq_chain
...
def context_filter(state):
    ...
    filtered_docs = documents_list_filter(
        state["docs"], filter_key="score", threshold=qd_match_threshold
    )
...
def decide_intent(state):
    ...
    if len(state["qq_result"]) > 0:
        state["answer"] = sorted(
            state["qq_result"], key=lambda x: x["score"], reverse=True
        )[0]["answer"]
...
def market_chain_knowledge_entry(
    ...
    workflow.add_node(
        "qq_match_and_intent_recognition", qq_match_and_intent_recognition
    )
    ...
    workflow.add_edge("query_preprocess", "qq_match_and_intent_recognition")
    ...
    workflow.add_conditional_edges(
        ...
        decide_intent,
        {"mkt_fast_reply": "mkt_fast_reply", "qd_retriver": "qd_retriver"},
    )

    workflow.add_conditional_edges(
        "context_filter",
        decide_if_context_sufficient,
        {"mkt_fast_reply": "mkt_fast_reply", "llm": "llm"},
    )
