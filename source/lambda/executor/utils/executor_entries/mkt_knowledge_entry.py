...
intent_recognition_embedding_endpoint = os.environ.get(
    "intent_recognition_embedding_endpoint", ""
)
...
def get_qd_chain(qd_config, qd_workspace_list):
    ...
    if reranker_type == RerankerType.BGE_RERANKER.value:
        compressor = BGEReranker(
            query_key=qd_query_key, enable_debug=qd_config["enable_debug"]
        )
...
def market_chain_knowledge_entry(
    ...
    # step 0 query reject#
    ######################
    query_length_threshold = rag_config["query_process_config"][
        "query_length_threshold"
    ]
    is_query_invalid_chain = RunnablePassthrough.assign(
    ...
    conversation_query_rewrite_config = rag_config["query_process_config"][
        "conversation_query_rewrite_config"
    ]
    conversation_query_rewrite_result_key = conversation_query_rewrite_config[
        "result_key"
    ]
    cqr_llm_chain = LLMChain.get_chain(
        intent_type=CONVERSATION_SUMMARY_TYPE, **conversation_query_rewrite_config
    )
    ...
    translate_chain = RunnableLambda(
        lambda x: query_translate(
            x["query"], lang=x["query_lang"], translate_config=translate_config
        )
    )
    ...
    service_names_recognition_chain = RunnableLambda(
        lambda x: get_service_name(x["query"])
    )

    preprocess_chain = (
        query_clean_chain
        | lang_check_and_translate_chain
        | RunnableParallelAssign(
    ...
    intent_postprocess_chain = intent_recognition_index.as_intent_postprocess_chain(
        method="top_1"
    )

    intent_search_and_postprocess_chain = (
        intent_index_search_chain | intent_postprocess_chain
    )
    intent_branch = RunnableBranch(
        (
            lambda x: not x["is_intent_index_exist"],
            intent_index_ingestion_chain | intent_search_and_postprocess_chain,
        ),
        intent_search_and_postprocess_chain,
    )
    ...
    # step 3.2 qq match#
    ####################
    qq_match_threshold = rag_config["retriever_config"]["qq_config"][
        "qq_match_threshold"
    ]
    ...
    llm_chain = RunnableDictAssign(
        lambda x: contexts_trunc(
            x["docs"], score_key="rerank_score", context_num=context_num
        )
    ) | RunnablePassthrough.assign(
    ...
    # step 6.1 rag chain #
    ######################
    qd_match_threshold = rag_config["retriever_config"]["qd_config"][
        "qd_match_threshold"
    ]
    qd_fast_reply_branch = RunnablePassthrough.assign(
        filtered_docs=RunnableLambda(
            lambda x: documents_list_filter(
                x["docs"], filter_key="rerank_score", threshold=qd_match_threshold
            )
        )
    ) | RunnableBranch(
        (
            ...
            RunnableLambda(
                lambda x: mkt_fast_reply(
                    x, fast_info="insufficient context", debug_info=debug_info
                )
            ),
        ),
        llm_chain,
    ...
    qd_chain = RunnableBranch(
        (lambda x: x["intent_type"] == IntentType.MARKET_EVENT.value, event_qd_chain),
        qd_chain,
    )

    rag_chain = chain_logger(
        qd_chain | qd_fast_reply_branch, "rag chain", trace_infos=trace_infos
    )
    ...
    allow_intents = [IntentType.KNOWLEDGE_QA.value, IntentType.MARKET_EVENT.value]
    qq_and_intent_fast_reply_branch = RunnableBranch(
        (
            ...
            RunnableLambda(
                lambda x: mkt_fast_reply(
                    x,
                    answer=sorted(
                        x["qq_result"], key=lambda x: x["score"], reverse=True
                    )[0]["answer"],
        ...
        RunnablePassthrough.assign(
            query=RunnableLambda(lambda x: rule_based_query_expansion(x["query"]))
        )
        | rag_chain,
    )
    ...
    full_chain = is_query_invalid_chain | RunnableBranch(
        (
    ...
    logger.info(
        f'session_id: {rag_config["session_id"]}, chain trace info:\n{trace_info}'
    )
