...
def market_chain_knowledge_entry(
    ...
    preprocess_chain = lang_check_and_translate_chain | RunnableParallelAssign(
        is_api_query=is_api_query_chain, service_names=service_names_recognition_chain
    )
    ...
    retriever_list = [
        QueryQuestionRetriever(
            workspace, size=qq_retriver_top_k, query_key=qq_query_key
        )
