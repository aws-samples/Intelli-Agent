...
def parse_market_conversation_summary_entry_config(event_body):
    ...
    llm_model_endpoint_name = event_body.get('llm_model_endpoint_name',llm_model_endpoint_name)
    assert llm_model_id and llm_model_endpoint_name, (llm_model_id,llm_model_endpoint_name)
    default_config = {
         "mkt_conversation_summary_config": {
...
def parse_mkt_entry_config(event_body):
    ...
    mkt_default_config = {
        ...
        "query_process_config":{
            ...
            "translate_config":{
                # default use Amazon Translate service
                "model_id": llm_model_id if is_cn_region else AWS_TRANSLATE_SERVICE_MODEL_ID,
                "endpoint_name": llm_model_endpoint_name
            }
        },
        # intent_config
    ...
    new_event_config = update_nest_dict(
        copy.deepcopy(mkt_default_config),
        event_body
        )
