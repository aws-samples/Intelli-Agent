...
def get_conversation_query_rewrite_chain(
    chat_history: list, conversation_query_rewrite_config
):
    # single turn
    if not chat_history:
...
def get_query_process_chain(chat_history, query_process_config, message_id=None):
    query_rewrite_config = query_process_config["query_rewrite_config"]
    conversation_query_rewrite_config = query_process_config[
    ...
    query_rewrite_chain = RunnablePassthrough.assign(
        query_rewrite=LLMChain.get_chain(
            query_key="query", intent_type=QUERY_REWRITE_TYPE, **query_rewrite_config
        )
    )
    ...
    hyde_chain = RunnablePassthrough.assign(
        hyde_doc=LLMChain.get_chain(
            intent_type=HYDE_TYPE, query_key="query", **hyde_config
        )
    )
