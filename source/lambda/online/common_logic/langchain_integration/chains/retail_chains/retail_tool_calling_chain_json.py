...
## Tips
   ...
   - 回答必须简洁，不允许出现超过2句话的回复。{{non_ask_rules}}"""
    ...
    def create_system_prompt(cls, goods_info: str, tools: list[dict], fewshot_examples: list, create_time=None) -> str:
        tool_descs = '\n\n'.join(
            cls.get_function_description(tool) for tool in tools)
        ...
        if create_time:
            ...
            logger.info(
                f"create_time: {create_time} is not valid, use current time instead.")
    ...
    def create_chat_history(cls, x, system_prompt=None):
        ...
        if chat_history[-1]['role'] == MessageType.AI_MESSAGE_TYPE:
            ...
            tool_calls = chat_history[-1].get("additional_kwargs",
                                              {}).get("tool_calls", [])
            if tool_calls:
    ...
    def parse_function_calls_from_ai_message(cls, message: dict):
        ...
        function_calls = re.findall(
            f"{cls.FN_NAME}.*?{cls.FN_RESULT}", content + stop_reason, re.S)
        return {
    ...
    def create_chain(cls, model_kwargs=None, **kwargs):
        ...
        model_kwargs["stop"] = model_kwargs.get(
