...
def agent(state: ChatbotState):
    ...
    if agent_tool_history and hasattr(agent_tool_history[-1], 'additional_kwargs'):
        search_result = agent_tool_history[-1]['additional_kwargs']['original'][0].get(
            'search_result', 1)
        if search_result == 0:
            context = agent_tool_history[-1]['additional_kwargs']['original'][0].get(
                'result', "")
        ...
        for hist in event_body["chat_history"]:
            if goods_id == hist['additional_kwargs']['goods_id']:
                ...
                if 'goods_id' in hist['additional_kwargs']:
                    current_chat['addional_kwargs']['goods_id'] = str(
                        hist['additional_kwargs']['goods_id'])
...
def retail_entry(event_body):
    ...
    event_body['chatbot_config'] = RetailConfigParser.from_chatbot_config(
    ...
    if goods_id:
        try:
            _goods_info = json.loads(goods_dict.get(
                goods_id, {}).get("goods_info", ""))
            _goods_type = goods_dict.get(goods_id, {}).get("goods_type", "")
        except Exception as e:
            import traceback
            error = traceback.format_exc()
            logger.error(
                f"error meesasge {error}, invalid goods_id: {goods_id}")
            _goods_info = None
