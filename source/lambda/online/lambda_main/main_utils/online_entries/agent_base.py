...
def tools_choose_and_results_generation(state):
    ...
    agent_repeated_call_validation = (
...
def results_evaluation(state):
    # parse tool_calls:
    try:
        output = _parse_tool_calling(agent_output=state["agent_current_output"])
        ...
        if not state["extra_response"].get("current_agent_intent_type", None):
            state["extra_response"]["current_agent_intent_type"] = output["tool_calls"][
                0
            ]["name"]
        ...
        return {
            ...
            "agent_tool_history": [e.agent_message, e.error_message],
        }
...
def tool_execution(state):
    ...
    for tool_call in tool_calls:
        ...
        output = invoke_lambda(
            event_body={"tool_name": tool_name, "state": state, "kwargs": tool_kwargs},
    ...
    return {"agent_tool_history": [output["tool_message"]]}
...
def build_agent_graph(chatbot_state_cls):
    def _results_evaluation_route(state: dict):
        ...
        if (
            ...
            and not state["function_calling_parse_ok"]
        ):
