...
def _test_multi_turns(user_queries, record_goods_id=False):
    ...
    for query in user_queries:
        ...
        create_time = query.get(
            'create_time', datetime.now().strftime(CREATE_TIME_FORMAT))
        try:
        ...
        r = generate_answer(
            ...
            chatbot_config={
                **chatbot_config, "goods_id": query.get("goods_id"), "create_time": create_time},
            entry_type="retail"
        )
...
def batch_test(data_file, count=1000, add_eval_score=True, record_goods_id=False):
        ...
        while True:
            ...
            if add_eval_score and datum['ai_msg'] and ground_truth:
                try:
                    ...
                    print('auto evaluation error: ', str(e),
                          (str(ground_truth), str(datum['ai_msg'])))
    ...
    for datum in tqdm.tqdm(data[:count], total=min(len(data), count)):
        ...
        chatbot_config.update(
            {"goods_id": product_ids, "create_time": create_time})
        try:
            r = generate_answer(
        ...
        datum['intention_fewshot_examples'] = r.get(
            'intention_fewshot_examples', None)
        if ai_msg:
