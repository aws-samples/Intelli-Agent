...
class RerankContentHandler(ContentHandlerBase):
    ...
    def transform_input(
        self, query: str, documents: List[str], model_kwargs: Dict
    ) -> bytes:
...
class SagemakerEndpointRerank(BaseDocumentCompressor):
    ...
    def rerank(self, documents: Sequence[Document], query: str):
    ...
    def compress_documents(
        ...
        for i in range(0, len(documents), self.rerank_batch_size):
            batch_documents = documents[i : i + self.rerank_batch_size]
            # rerank
            rerank_scores = self.rerank(documents=batch_documents, query=query)
...
class SageMakerMultiModelRerankModelBase(RerankModelBase):
    ...
    def create_model(cls, **kwargs):
        ...
        # Create a new dictionary for endpoint_kwargs instead of trying to copy the Field
        endpoint_kwargs = {}
        if hasattr(cls, "endpoint_kwargs") and isinstance(
            cls.endpoint_kwargs, dict
        ):
            endpoint_kwargs = dict(cls.endpoint_kwargs)
